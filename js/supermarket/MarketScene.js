...
export class Market3d {
    ...
    async initScene() {
        ...
        this.camera.name = "Market-camera"
       ...
       await LoadModel('superMarket.glb', this.scene,this.renderer,this.camera,this.wallBoundingBoxes) 
       ...
       const light = new THREE.AmbientLight( 0x404040,2 ); // soft white light
        ...
        const annotation = document.querySelector('.annotationMarket');
...
loader.setDRACOLoader(loaderDRACO);
 ...
 async function LoadModel(model, _thisScene,_thisrenderer,_thiscamera,wallBoundingBoxes) {
    return new Promise((resolve, reject) => {
        ...
        xhr => {
          console.log(`Load Model Market :  ${Math.floor((xhr.loaded / xhr.total) * 100)}% loaded`);
          _("loadertxtMarket").innerText = Math.floor((xhr.loaded / xhr.total) * 100) + "%"
        },
        err => {
    ...
    return new Promise((resolve, reject) => {
        var loaderhdr = new RGBELoader().setDataType(THREE.UnsignedByteType).setPath('./img/env/').load(
          hdr,async function (texture) {
              ...
              _("loadingMarket").style.display = "none";
              resolve()
            console.log("end Loading Market .... "+hdr);
          },
          xhr => {
            _("loadertxtMarket").innerText =Math.floor((xhr.loaded / xhr.total) * 100)+ "%"
          },
